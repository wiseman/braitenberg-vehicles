...
; +++ don't bother with this if Scheme.saveMacros is false
(define (unmacro item)
...
;;; here is generic-write

; display? #t --> print-readable (but it's broken for some reason)
(define (generic-write obj display? width output)

;  (print `(generic-write ,obj ,display? ,output))
  (define (read-macro? l)
    (define (length1? l) (and (pair? l) (null? (cdr l))))
    (let ((head (car l)) (tail (cdr l)))
      (case head
        ((quote quasiquote unquote unquote-splicing) #t)
        (else
	 #f))))
