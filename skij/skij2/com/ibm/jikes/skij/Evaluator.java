  ...
  static Object eval1(Object exp, Environment env) throws SchemeException {
	...
	if (qproc instanceof Symbol) { // check for special forms
	  ...
	  if (sym == "begin") {
	    if (args == Nil.nil)
	      return null;
	    for (; args.cdr != Nil.nil; args = (Cons)args.cdr)
	      eval(args.car, env);
	    exp = args.car;
	  }
	  else if (sym == "quote") 
	    ...
	    if (schemeTrue(eval(args.car, env)))
	      ...
	      if (args.kddr() == Nil.nil) // make sure third clause is present
		return null;
	      else
		exp = args.caddr(); 
	  } 
	...
	if (proc != null) {
	  ...
	  if (proc instanceof FExpr) {
	    Object expansion = proc.apply(env, list);

	    // old expander
// 	    list.cdr = saveMacroSource ? 
// 	      Cons.list(Cons.list(Symbol.QUOTE,
...
// 	    exp = expansion;
  ...
  // not usually used
  static public Object evalProg(Cons progbody, Environment env) throws SchemeException {
    Object result = null;
