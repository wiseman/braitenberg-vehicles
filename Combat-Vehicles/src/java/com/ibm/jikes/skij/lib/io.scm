...
(define (call-with-flushing-delayed port func)
  (dynamic-wind
   (lambda ()
     (poke port 'activeFlushing #f))
   func
   (lambda ()
     (poke port 'activeFlushing #t)
     (invoke port 'flush))))

(defmacro (with-flushing-delayed port . body)
  `(call-with-flushing-delayed ,port
                               (lambda ()
                                 ,@body)))
...
(define print
  (lambda args
    (require-write)
    (with-flushing-delayed (current-output-port)
      (apply write args)
