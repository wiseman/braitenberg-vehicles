...
(define (get-method obj mname . args)
  (invoke-static 'com.ibm.jikes.skij.util.Dynvoke 'getMethod
		 obj
		 (string mname)
...
; +++ for jdk1.2, find non-public methods
(define (method-apropos class mname)
  (set! mname (to-string mname))
  (for-vector (lambda (method)
		(if (>= (invoke (invoke method 'getName) 'indexOf mname) 0)
		    (print method)))
	      (invoke (coerce-class class) 'getMethods)))
...
(define (trace on?)
  (invoke-static 'com.ibm.jikes.skij.util.Tracer 'setOn on?))
